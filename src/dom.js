...
const dom = ( () => { 
    ...
    function formatTime(data, units) {
      ...
      // Ensure that data is a valid Date object
      if (typeof data === 'number') {
        // If `data` is a Unix timestamp (number), convert it to a Date
        data = new Date(data * 1000); // Convert to milliseconds
      } else if (typeof data === 'string') {
        // If `data` is a string, attempt to parse it as a date
        data = new Date(data);
      }
    
      // Check if the date is valid
      if (isNaN(data)) {
        console.error("Invalid date value:", data);
